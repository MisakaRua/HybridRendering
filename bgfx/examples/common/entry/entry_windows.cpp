				...
				case WM_SIZING:
					{
						...
						&&  ENTRY_WINDOW_FLAG_ASPECT_RATIO & m_flags[handle.idx])
						{
							...
							case WMSZ_RIGHT:
								{
									float m_aspect_ratio = 1.0f/m_aspectRatio;
									width  = bx::uint32_max(ENTRY_DEFAULT_WIDTH/4, width);
									height = uint32_t(float(width)*m_aspect_ratio);
								}
								break;

							default:
								{
									float m_aspect_ratio = m_aspectRatio;
									height = bx::uint32_max(ENTRY_DEFAULT_HEIGHT/4, height);
									width  = uint32_t(float(height)*m_aspect_ratio);
								}
								break;
							}
