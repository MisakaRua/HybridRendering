...
struct DebugDrawEncoderImpl
{
	...
	void drawQuad(SpriteHandle _handle, const bx::Vec3& _normal, const bx::Vec3& _center, float _size)
	{
		...
		const float m_aspect_ratio = float(pack.m_width)/float(pack.m_height);
		const float halfExtentU =      m_aspect_ratio*_size*0.5f;
		const float halfExtentV = 1.0f/m_aspect_ratio*_size*0.5f;
